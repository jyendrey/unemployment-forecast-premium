#!/usr/bin/env python3
"""
Test Recalibrated Forecast
Tests the forecast with recalibrated weights after removing redundant factors
"""
...
def test_recalibrated_forecast():
    """Test the forecast with recalibrated weights"""
    
    print("🧪 Testing Recalibrated Forecast (No Redundant Factors)")
    print("="*80)
    ...
    try:
        ...
        print("🔄 Calculating enhanced forecast with recalibrated weights...")
        ...
        # Print results
        print("\n" + "="*80)
        print("🎯 RECALIBRATED FORECAST RESULTS")
        print("="*80)
        ...
        previous_forecast = 4.25  # From previous forecast with all factors
        forecast_change = forecast - previous_forecast
        
        print(f"\n📊 Forecast Comparison:")
        print(f"   • Previous Forecast (All Factors): 4.25%")
        print(f"   • New Forecast (Recalibrated): {forecast:.2f}%")
        print(f"   • Change from Previous: {forecast_change:+.4f}%")
        print(f"   • Change from Original 4.2%: {forecast - 4.2:+.4f}%")
        
        # Show removed factors
        print(f"\n🗑️ Removed Redundant Factors:")
        print(f"   • Claims Trend: 0.0000% (was 0.0005% weight)")
        print(f"   • Market Stability: 0.0000% (was 0.0003% weight)")
        print(f"   • Economic Health: 0.0000% (was 0.0015% weight)")
        print(f"   • Economic Risk: 0.0000% (was 0.001% weight)")
        ...
        with open('recalibrated_forecast_results.json', 'w') as f:
            json.dump(test_results, f, indent=2)
        
        print(f"\n✅ Test results saved to: recalibrated_forecast_results.json")
        
        # Print summary
        print(f"\n📋 RECALIBRATED FORECAST SUMMARY:")
        print(f"   • Final Forecast: {forecast:.2f}%")
        print(f"   • Enhanced Confidence: {confidence:.1f}%")
        print(f"   • Factors Removed: 4 (redundant)")
        print(f"   • Factors Remaining: {len(adjustments)}")
        ...
        print(f"❌ Error running recalibrated forecast: {e}")
        import traceback
        traceback.print_exc()
        return None

if __name__ == "__main__":
    results = test_recalibrated_forecast()
    if results:
        print(f"\n🎉 Recalibrated forecast completed successfully!")
    else:
        print(f"\n💥 Recalibrated forecast failed!")