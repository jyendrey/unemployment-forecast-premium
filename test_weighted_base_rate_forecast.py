#!/usr/bin/env python3
"""
Test Weighted Base Rate Forecast
Tests the forecast with the new weighted base rate of 4.2485%
"""
...
def test_weighted_base_rate_forecast():
    """Test the forecast with weighted base rate"""
    
    print("🧪 Testing Weighted Base Rate Forecast")
    print("="*80)
    ...
    try:
        ...
        print("🔄 Calculating enhanced forecast with weighted base rate...")
        ...
        # Print results
        print("\n" + "="*80)
        print("🎯 WEIGHTED BASE RATE FORECAST RESULTS")
        print("="*80)
        ...
        # Compare with previous base rate
        previous_base_rate = 4.2
        weighted_base_rate = 4.2485
        base_rate_change = weighted_base_rate - previous_base_rate
        
        print(f"\n📊 Base Rate Comparison:")
        print(f"   • Previous Base Rate: {previous_base_rate}%")
        print(f"   • Weighted Base Rate: {weighted_base_rate:.4f}%")
        print(f"   • Base Rate Change: {base_rate_change:+.4f}%")
        print(f"   • Final Forecast: {forecast:.2f}%")
        print(f"   • Total Change from 4.2%: {forecast - 4.2:+.4f}%")
        ...
        test_results = {
            ...
            "base_rate_analysis": {
                "previous_base_rate": previous_base_rate,
                "weighted_base_rate": weighted_base_rate,
                "base_rate_change": base_rate_change,
                "total_change_from_4_2": forecast - 4.2
            },
            "factor_impacts": {
                "lfpr": lfpr_adj,
                "claims": claims_adj,
        ...
        with open('weighted_base_rate_forecast_results.json', 'w') as f:
            json.dump(test_results, f, indent=2)
        
        print(f"\n✅ Test results saved to: weighted_base_rate_forecast_results.json")
        ...
        print(f"❌ Error running weighted base rate forecast: {e}")
        import traceback
        traceback.print_exc()
        return None

if __name__ == "__main__":
    results = test_weighted_base_rate_forecast()
    if results:
        print(f"\n🎉 Weighted base rate forecast completed successfully!")
    else:
        print(f"\n💥 Weighted base rate forecast failed!")